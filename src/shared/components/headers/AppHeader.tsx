...
import { ChartPie, Home, Library, Plus, Settings as SettingsIcon } from 'lucide-react';
    ...
    <>
      <header className="sticky top-0 z-40 flex items-center justify-between bg-app py-2.5 sm:py-3">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="rounded-lg border border-subtle px-3 text-sm inline-flex items-center justify-center h-10 transition-colors duration-150 hover-nonaccent"
            onClick={() => openCreate(location.pathname === '/' ? 'reading' : 'next')}
            aria-label="Add book"
            title="Add book"
          >
